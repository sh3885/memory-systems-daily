...
function buildDraftPrompt({ lessonDate, lesson }) {
  ...
  return [
    ...
    "3. `## 주제에 맞는 핵심 설명` 안에서 내용을 나눌 때는 소제목을 `### ▎**소제목**` 형식으로 쓰고, 각 소제목 바로 앞에 빈 줄과 `---` 구분선을 넣는다. `##` 섹션 제목에는 `▎`나 굵게 표시를 넣지 않는다.",
    "4. 구조, 흐름, 데이터 이동, 비교 관계를 이해하는 데 그림이 도움이 되면 다이어그램을 생략하거나 표·텍스트로 대체하지 않는다. 다이어그램을 하나 이상 넣는다. 다이어그램은 Markdown 코드블록 밖의 완전한 `<svg>...</svg>`로 작성한다. 게시 과정이 이를 실제 이미지로 변환하므로 독자 화면에는 SVG 코드나 `<text ...>`가 보이지 않는다. SVG 외의 HTML, Mermaid, XML 코드는 넣지 않는다.",
    "5. 중요한 사실은 문장 안에서 출처를 자연스럽게 밝힌다. 검증 과정이나 작성 메모를 별도 부록 섹션으로 만들지 않는다.",
    "6. 글의 마지막 섹션은 `## 자주 묻는 질문`으로 하고, 실무적인 질문과 답변을 최소 두 개 넣는다. '다음 질문' 섹션은 만들지 않는다.",
    "7. 먼저 초안을 완성한 뒤, 최종 답변 전에 독자 관점에서 한 번 검토한다. 핵심 설명의 정확성·누락·흐름·예시의 적절성·독자가 바로 이해할 수 있는 표현·금지 형식이 없는지를 확인하고, 발견한 개선점을 반영해 글 전체를 다시 다듬는다. 검토 과정, 체크리스트, 초안은 출력하지 않는다.",
    "8. 완성된 결과는 반드시 draft.md 파일 하나로 제공한다. 채팅 본문에는 파일 내용의 요약이나 코드블록을 반복하지 않는다.",
    "",
    "출력 형식:",
    "# 제목",
    "## 오늘의 질문",
