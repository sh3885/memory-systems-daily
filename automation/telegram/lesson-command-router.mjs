...
function buildDraftPrompt({ lessonDate, lesson }) {
  ...
  return [
    ...
    "3. 계산, 표, 실험, 다이어그램은 위의 산출물 선택 기준을 통과할 때만 넣는다. SVG, HTML, Mermaid, XML 등 그림 코드는 절대 본문에 넣지 않는다.",
    ...
    "6. 완성된 결과는 반드시 draft.md 파일 하나로 제공한다. 채팅 본문에는 파일 내용의 요약이나 코드블록을 반복하지 않는다.",
    "",
    "출력 형식:",
    "# 제목",
    "## 오늘의 질문",
