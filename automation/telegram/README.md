...
`telegram-client.mjs` wraps `sendMessage` and `answerCallbackQuery` with JSON POST requests. `lesson-command-router.mjs` handles lesson selection, `/prompt`, final Markdown upload through `/draft`, `/review`, `/help`, and plain-text Q&A. `worker.mjs` expects a D1 binding named `DB` and the environment values in `.env.example`, including `DAILY_CURRICULUM_REF` until a full curriculum selector is wired in.
